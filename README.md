...
**Installing ISO files and preforming an SHA256 hash**
...
**Loading ISO files into Virtualbox**
...
**Configure network settings to be on the same network** 
...
Now attach the network to each VM by pressing the VM and then the yellow wheel settings icon at the top. Go to the network tab and in the dropdown box select `NAT NETWORK` and the name dropdown box should be the one that was created `ADproject`.
...
**Apply static IP address to each machine. Each VM's IPv4 address will reflect the network diagram**
...
**Create new organiational unit called IT** <br>
...
<img src="https://i.imgur.com/8435dDA.png" width="400"> <br> <sup>Ref 14: Creation of new Organiational Unit called IT </sup>	 
...
Right-click the user and select reset password. It will prompt you to create a new password, Check the box to make the user change to a password of their choice the next time they logon. 
...
Right click account lockout threshold and select properties. Here is where you can edit the number of times of wrong login attempts can occur before the account will be locked out.
...
<img src="https://i.imgur.com/3xuqS37.png" width="470"> <img src="https://i.imgur.com/DBeIxAB.png" width="500"> <br> <sup>Ref 20: Sign in to domain with user that has been created  </sup>	
